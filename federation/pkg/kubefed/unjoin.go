...
func deleteConfigMapFromCluster(hostClientset *internalclientset.Clientset, secret *api.Secret, cluster *federationapi.Cluster, fedNamespace string) error {
	clientset, err := getClientsetFromCluster(secret, cluster)
	if err != nil {
		return err
	}

	configMap, err := clientset.Core().ConfigMaps(metav1.NamespaceSystem).Get(util.KubeDnsName, metav1.GetOptions{})
	if err != nil {
		return err
	}
	...
	if configMap.Data != nil && len(configMap.Data) > 0 {
		newFedMapValue = removeConfigMapString(configMap.Data[kubefedinit.FedConfigMapKey], cmDep.Annotations[kubefedinit.FedConfigMapKey])
	} else {
		// Configmap somehow already does not have data
		// should be removed
		return clientset.Core().ConfigMaps(metav1.NamespaceSystem).Delete(util.KubeDnsName, &metav1.DeleteOptions{})
	}

	if  newFedMapValue != "" {
		configMap.Data[kubefedinit.FedConfigMapKey] = newFedMapValue
...
// or returns the remaining comma saperated strings minus the one to be removed
