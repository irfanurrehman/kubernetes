...
func TestAutoscalerController(t *testing.T) {
	...
	hpaController.autoscalerReviewDelay = 50 * time.Millisecond
	hpaController.clusterAvailableDelay = time.Second
	hpaController.updateTimeout = 5 * time.Second
	informer := ToFederatedInformerForTestOnly(hpaController.fedAutoscalerInformer)
	...
	assert.True(t, hpaController.hasFinalizerFunc(updatedHpa, metav1.FinalizerOrphanDependents))
	hpa1 = *updatedHpa
	...
	assert.True(t, hpaIsEqual(hpa1, *createdHpa),
	...
	hpa1.Spec = autoscalingv1.HorizontalPodAutoscalerSpec{
		ScaleTargetRef: autoscalingv1.CrossVersionObjectReference{
			Kind: "HorizontalPodAutoscalerController",
			Name: "anotherhpa",
		},
	}
	...
	assert.True(t, hpaIsEqual(hpa1, *createdHpa2),
